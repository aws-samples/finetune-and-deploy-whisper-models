...
def prepare_dataset(batch, processor, device):
    ...
    batch['input_features'] = torch.tensor(input_features, dtype=torch.float16).to(device)
    batch['labels'] = torch.tensor(
        processor.tokenizer(batch['sentence']).input_ids, dtype=torch.long).to(device)
    return batch
...
if __name__ == "__main__":
    ...
    dataset = load_dataset(args.train, 'successful_texts.json')
    ...
    # Save loss log
    with open(os.path.join(args.output_data_dir, 'loss_log.json'), 'w') as f:
